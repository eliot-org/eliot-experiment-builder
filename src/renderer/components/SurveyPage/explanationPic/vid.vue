<template>
    <figure id="videoContainer" ref="videoContainer" data-fullscreen="false" @click="video()">
        <video id="video" ref="video" v-bind:class="(fullscreen == true) ? 'fullscreen-logo' : 'logo'" controls>
            <source :src="src + '?' + Date.now()"  type="video/mp4">
            Your browser does not support the video tag.
        </video> 
        <div id="videoControls" ref="videoControls" class="controls" data-state="hidden">
            <button id="playpause" ref="playpause" type="button" data-state="play">Play/Pause</button>
            <button id="stop" ref="stop" type="button" data-state="stop">Stop</button>
            <div class="progress">
                <progress id="progress" ref="progress" value="0" min="0">
                    <span id="progressBar" ref="progressBar"></span>
                </progress>
            </div>
            <button id="mute" ref="mute" type="button" data-state="mute">Mute/Unmute</button>
            <button id="volinc" ref="volinc" type="button" data-state="volup">Vol+</button>
            <button id="voldec" ref="voldec" type="button" data-state="voldown">Vol-</button>
            <button id="skip" ref="skip" type="button" data-state="skip" class="skipBtn">Skip</button>
            <button id="fs" ref="fs" type="button" data-state="go-fullscreen" v-if="0">Fullscreen</button>
        </div>
    </figure>
</template>
    ...
    methods:{
        video(){
            ...
            if (supportsVideo) {
                ...
                var playpause = this.$refs.playpause;
                var stop = this.$refs.stop
                var mute = this.$refs.mute;
                var volinc = this.$refs.volinc;
                var voldec = this.$refs.voldec;
                var skip = this.$refs.skip;
                var progress = this.$refs.progress;
                var progressBar = this.$refs.progressBar
                //var fullscreen = this.$refs.fs;
                ...
                var checkVolume = function(dir) {
                    if (dir && !volTimeout) {
                        volTimeout = true
                        setTimeout(function() {volTimeout = false}, 250);
...
.controls progress::-moz-progressBar {
